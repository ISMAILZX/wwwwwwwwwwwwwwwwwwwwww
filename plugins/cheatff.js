let handler = async (m, { conn }) => {
    ...
    if (new Date - global.DATABASE._data.users[m.sender].lastclaim > 43200000) {
        conn.reply(m.chat, `Kamu sudah mengclaim dan mendapatkan *1000000000* 💵money dan *1000000000* 🥤potion`, m)
        user.money += 1000000000
        user.potion += 1000000000
        user.lastclaim = new Date * 1
    } else conn.reply(m.chat, `silahkan tunggu *${timers}* lagi untuk bisa mengclaim lagi`, m)
}
handler.help = ['cheatff']
handler.tags = ['rpg']
handler.command = /^(citff)$/i
